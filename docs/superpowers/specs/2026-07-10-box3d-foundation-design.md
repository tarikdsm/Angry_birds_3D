...
Todos os comandos que alteram o mundo durante uma simulação são enfileirados. `step()` aplica a fila antes da gravidade, avança Box3D, copia eventos e publica um snapshot contíguo. A API não expõe ponteiros do solver nem referências aos eventos transitórios do Box3D. `states()`, `contact_hits()` e `joint_reactions()` devolvem `std::span<const T>` sobre buffers publicados e pertencentes a `PhysicsWorld`; essas views servem apenas para leitura imediata e podem ser invalidadas por qualquer operação não `const` no mundo, além de move ou destruição. Consumidores que precisem reter dados além desse limite devem copiá-los. `ScenarioBuilder` constrói stress, hulls, compounds e juntas exclusivamente por essa API; os testes não podem acessar `b3*` diretamente, exceto um teste de conformidade isolado do adaptador.
...
No jogo normal, o node avança pelo relógio físico do Godot usando acumulador fixo e publica snapshots. A cena mínima instancia meshes primitivos para cada handle e atualiza seus transforms. O planeta visual é uma esfera simples; a torre usa caixas coloridas; o projétil usa outra esfera. O Godot não possui corpos físicos correspondentes. Nesta cena de regressão, o planeta é deliberadamente o único corpo estático e, portanto, o único snapshot com massa zero; a view pode usar esse invariante para associar `planet_view`. Se a superfície pública do spike passar a criar outro corpo estático, `get_body_states()` deve antes publicar uma identificação explícita do planeta, em vez de ampliar a heurística.
...
Após a execução completa e a validação do payload, `tools/run_spike.ps1` acrescenta a identidade do código testado: `source_revision` contém o `HEAD` no início da certificação, `tested_inputs_schema=ninho.tested-inputs.v2`, `tested_inputs[]` registra tamanho e SHA-256 do conjunto canônico, e `tested_inputs_sha256` agrega esse manifesto com texto UTF-8 canonizado para LF e binários byte a byte. O conjunto inclui todos os arquivos sob `native/` e `cmake/`, `CMakeLists.txt`, `CMakePresets.json`, o lock da toolchain e a allowlist dos scripts e módulos diretos ou transitivos que constroem, executam, geram e validam a evidência da foundation. Entradas obrigatórias ausentes falham de forma fechada; caminhos são normalizados com `/`, deduplicados e ordenados ordinalmente antes do hash. Outputs gerados sob `build/`, `artifacts/` e `docs/physics/evidence/`, além do relatório Markdown gerado, permanecem fora da identidade para evitar dependência circular. O gate exige que a revisão seja ancestral do `HEAD` atual e que o manifesto coincida com a árvore de trabalho. Assim um artifact local gerado sobre alterações ainda não commitadas continua válido enquanto seus bytes não mudarem; qualquer drift posterior em um insumo canônico invalida a evidência. A publicação dos dois snapshots normativos é explícita por `-PublishCanonicalEvidence`, exige identidades Debug/Release iguais e só ocorre quando ambos os artifacts locais completos passam os gates.

`artifacts/physics/` contém exclusivamente outputs locais voláteis ignorados pelo Git: JSONs de trabalho, logs e filmes são criados ou substituídos pelos runners e podem desaparecer em uma limpeza. Portanto, o diretório ausente ou vazio é estado esperado em um clone limpo e não invalida a fundação. A publicação copia somente os dois JSONs validados para `docs/physics/evidence/foundation-report-{debug,release}.json`; esses snapshots rastreados, referenciados pelos seus hashes no relatório gerado, são a evidência normativa. Caminhos sob `build/` e `artifacts/physics/` documentam onde inspecionar a execução local quando esses arquivos ainda existem, não uma promessa de retenção.
...
Box3D, godot-cpp e extensão usam a mesma arquitetura, toolset, Windows SDK e CRT. A extensão declara `compatibility_minimum = "4.5"` e usa a `extension_api.json` do tag fixado do godot-cpp. O contrato de compatibilidade deste marco é a API minor 4.5: o host 4.5.1 não exige um tag patch-specific do godot-cpp. O pin só deve mudar numa migração deliberada da versão minor do Godot ou para incorporar uma correção upstream relevante, sempre repetindo os gates de build, carregamento da DLL e smokes no host fixado. O smoke test do marco verifica ABI no Godot 4.5.1; o marco de entrega repete o pacote em VMs limpas Windows 10/11.
